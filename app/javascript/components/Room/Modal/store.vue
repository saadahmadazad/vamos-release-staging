...
<script>
  ...
  export default {
    ...
    methods: {
      async initOption () {
        // 施設 data
        let data = await this.axios.get('/api/v1/facilities')
                                    .then(res => res.data)
                                    .catch(error => this.alert = { type: error.data.status, message: error.data.message })
        this.facilities = data.facilities

        // 施設 OTA 部屋 Optionデーターを取り出す
        this.axios.get('/api/v1/ota/?facility=' + data.facilities[0].id)
        .then((res) => {
