<template>
  <el-dialog title="OTA連携編集" :visible.sync="dialogStatus" :before-close="toggleModal">
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="Create('ruleForm')">編集</el-button>
    </div>
  </el-dialog>
</template>
...
<script>
  export default {
    ...
    data() {
      return {
        data: {
          provider: this.updateData.provider,
          account: this.updateData.account,
    ...
    methods: {
      ...
      Create (formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.axios.put('/api/v1/ota/'+ this.updateData.id, {
              otum: this.data
            })
            .then(res => {
              // Clear data
